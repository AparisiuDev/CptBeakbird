...
public class FoxAttack : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (foxMovement.player != null)
        {
            health = foxMovement.player.GetComponent<PlayerHealth>();
        }
    }
    ...
    void Update()
    {
        ...
        if (foxMovement == null || foxMovement.player == null || health == null)
            return;
        ...
        if (health.IsDead())
        {
            ...
            foxMovement.ReturnToPatrol();
            return;
        }

        float distanceToPlayer = Vector3.Distance(transform.position, foxMovement.player.position);
